...
import { loadMockData, saveMockData, type MockUser, type MockDataset, type MockOrganization, type MockTheme, type MockLicense } from '@/lib/mock/data'
 ...
 */
function mockResponse<T>(data: T, message?: string): Promise<ApiResponse<T>> {
 ...
 */
function mockError(message: string, statusCode: number = 400): Promise<ApiResponse<never>> {
  return delay(300).then(() => ({
    success: false,
    error: message,
    statusCode,
...
export const mockUsersApi = {
  async getAll(filters?: {
    ...
    page?: number
    limit?: number
  }): Promise<ApiResponse<{ users: Omit<MockUser, 'password'>[]; pagination: PaginationResponse }>> {
  ...
  async getById(id: string): Promise<ApiResponse<Omit<MockUser, 'password'>>> {
    const data = loadMockData()
...
export const mockDatasetsApi = {
  async getAll(filters?: {
    ...
    page?: number
    limit?: number
  }): Promise<ApiResponse<{ datasets: MockDataset[]; pagination: PaginationResponse }>> {
    const data = loadMockData()
  ...
  async getById(id: string): Promise<ApiResponse<MockDataset>> {
    const data = loadMockData()
    const currentUser = getMockUser()
...
export const mockOrganizationsApi = {
  async getAll(filters?: { search?: string; page?: number; limit?: number }): Promise<ApiResponse<{ organizations: MockOrganization[]; pagination: PaginationResponse }>> {
    const data = loadMockData()
  ...
  async getById(id: string): Promise<ApiResponse<MockOrganization>> {
    const data = loadMockData()
...
export const mockThemesApi = {
  async getAll(filters?: { search?: string; page?: number; limit?: number }): Promise<ApiResponse<{ themes: MockTheme[]; pagination: PaginationResponse }>> {
    const data = loadMockData()
  ...
  async getById(id: string): Promise<ApiResponse<MockTheme>> {
    const data = loadMockData()
...
export const mockLicensesApi = {
  async getAll(filters?: { search?: string; page?: number; limit?: number }): Promise<ApiResponse<{ licenses: MockLicense[]; pagination: PaginationResponse }>> {
    const data = loadMockData()
  ...
  async getById(id: string): Promise<ApiResponse<MockLicense>> {
    const data = loadMockData()
...
export const mockStatsApi = {
  async getStats(): Promise<ApiResponse<{
    ...
    recentDatasets: MockDataset[]
    topDatasets: MockDataset[]
  }>> {
    const data = loadMockData()
...
export const mockSessionApi = {
  async getSession(): Promise<ApiResponse<{
    ...
    const { getMockUser, getMockSession } = await import('@/lib/mock/auth')
